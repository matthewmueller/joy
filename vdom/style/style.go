...
func New(props *Props, children ...vdom.Child) *Style {
	macro.Rewrite("$1('style', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Scoped(scoped string) *Props {
	macro.Rewrite("$1().Set('scoped', $2)", macro.Runtime("Map", "Set", "JSON"), scoped)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Scoped(scoped string) *Props {
	macro.Rewrite("$_.Set('scoped', $1)", scoped)
