...
func New(props *Props, children ...vdom.Child) *Track {
	macro.Rewrite("$1('track', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Default(def string) *Props {
	macro.Rewrite("$1().Set('default', $2)", macro.Runtime("Map", "Set", "JSON"), def)
...
func (p *Props) Default(def string) *Props {
	macro.Rewrite("$_.Set('default', $1)", def)
	p.attrs["default"] = def
	return p
}
...
func Label(label string) *Props {
	macro.Rewrite("$1().Set('label', $2)", macro.Runtime("Map", "Set", "JSON"), label)
	p := &Props{attrs: map[string]interface{}{}}
...
func Srclang(srclang string) *Props {
	macro.Rewrite("$1().Set('srclang', $2)", macro.Runtime("Map", "Set", "JSON"), srclang)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Srclang(srclang string) *Props {
	macro.Rewrite("$_.Set('srclang', $1)", srclang)
