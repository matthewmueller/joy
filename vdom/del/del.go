...
func New(props *Props, children ...vdom.Child) *Del {
	macro.Rewrite("$1('del', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Cite(cite string) *Props {
	macro.Rewrite("$1().Set('cite', $2)", macro.Runtime("Map", "Set", "JSON"), cite)
...
func (p *Props) Cite(cite string) *Props {
	macro.Rewrite("$_.Set('cite', $1)", cite)
	p.attrs["cite"] = cite
	return p
}
...
func Datetime(datetime string) *Props {
	macro.Rewrite("$1().Set('datetime', $2)", macro.Runtime("Map", "Set", "JSON"), datetime)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Datetime(datetime string) *Props {
	macro.Rewrite("$_.Set('datetime', $1)", datetime)
