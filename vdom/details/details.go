...
func New(props *Props, children ...vdom.Child) *Details {
	macro.Rewrite("$1('details', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Open(open string) *Props {
	macro.Rewrite("$1().Set('open', $2)", macro.Runtime("Map", "Set", "JSON"), open)
...
func (p *Props) Open(open string) *Props {
	macro.Rewrite("$_.Set('open', $1)", open)
	p.attrs["open"] = open
	return p
}
