...
// Sortable fn
func Sortable(sortable string) *Props {
	macro.Rewrite("$1().Set('sortable', $2)", macro.Runtime("Map", "Set", "JSON"), sortable)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Sortable(sortable)
}

// Sortable fn
func (p *Props) Sortable(sortable string) *Props {
	macro.Rewrite("$_.Set('sortable', $1)", sortable)
	p.attrs["sortable"] = sortable
	return p
}

// Border fn
func Border(border string) *Props {
	macro.Rewrite("$1().Set('border', $2)", macro.Runtime("Map", "Set", "JSON"), border)
	p := &Props{attrs: map[string]interface{}{}}
