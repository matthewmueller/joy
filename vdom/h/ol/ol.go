...
// New fn
func New(props *Props, children ...vdom.Child) *Ol {
	js.Rewrite("$1('ol', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Ol) Render() vdom.Node {
	return s
}

func (s *Ol) String() string {
	// props
	var props []string
	...
	if len(props) > 0 {
		return "<ol " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</ol>"
	}

	return "<ol>" + strings.Join(children, "") + "</ol>"
}
...
// Compact fn
func Compact(compact string) *Props {
	js.Rewrite("$1().Set('compact', $2)", js.Runtime("Map", "Set", "JSON"), compact)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Compact(compact)
}

// Compact fn
func (p *Props) Compact(compact string) *Props {
	js.Rewrite("$_.Set('compact', $1)", compact)
	p.attrs["compact"] = compact
	return p
}

// Reversed fn
func Reversed(reversed string) *Props {
	js.Rewrite("$1().Set('reversed', $2)", js.Runtime("Map", "Set", "JSON"), reversed)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Reversed(reversed)
}

// Reversed fn
func (p *Props) Reversed(reversed string) *Props {
	js.Rewrite("$_.Set('reversed', $1)", reversed)
	p.attrs["reversed"] = reversed
	return p
}

// Start fn
func Start(start string) *Props {
	js.Rewrite("$1().Set('start', $2)", js.Runtime("Map", "Set", "JSON"), start)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Start(start)
}

// Start fn
func (p *Props) Start(start string) *Props {
	js.Rewrite("$_.Set('start', $1)", start)
	p.attrs["start"] = start
	return p
}
