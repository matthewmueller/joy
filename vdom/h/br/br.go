...
// New fn
func New(props *Props, children ...vdom.Child) *Br {
	js.Rewrite("$1('br', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Br) String() string {
	...
	if len(props) > 0 {
		return "<br " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</br>"
	}

	return "<br>" + strings.Join(children, "") + "</br>"
}
...
// Clear fn
func Clear(clear string) *Props {
	js.Rewrite("$1().Set('clear', $2)", js.Runtime("Map", "Set", "JSON"), clear)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Clear(clear)
}

// Clear fn
func (p *Props) Clear(clear string) *Props {
	js.Rewrite("$_.Set('clear', $1)", clear)
	p.attrs["clear"] = clear
	return p
}
