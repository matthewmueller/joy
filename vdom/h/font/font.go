...
// New fn
func New(props *Props, children ...vdom.Child) *Font {
	js.Rewrite("$1('font', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Font) String() string {
	...
	if len(props) > 0 {
		return "<font " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</font>"
	}

	return "<font>" + strings.Join(children, "") + "</font>"
}
...
// Color fn
func Color(color string) *Props {
	js.Rewrite("$1().Set('color', $2)", js.Runtime("Map", "Set", "JSON"), color)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Color(color)
}

// Color fn
func (p *Props) Color(color string) *Props {
	js.Rewrite("$_.Set('color', $1)", color)
	p.attrs["color"] = color
	return p
}

// Face fn
func Face(face string) *Props {
	js.Rewrite("$1().Set('face', $2)", js.Runtime("Map", "Set", "JSON"), face)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Face(face)
}

// Face fn
func (p *Props) Face(face string) *Props {
	js.Rewrite("$_.Set('face', $1)", face)
	p.attrs["face"] = face
	return p
}
