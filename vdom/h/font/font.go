...
func New(props *Props, children ...vdom.Child) *Font {
	macro.Rewrite("$1('font', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Color(color string) *Props {
	macro.Rewrite("$1().Set('color', $2)", js.Runtime("Map", "Set", "JSON"), color)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Color(color string) *Props {
	macro.Rewrite("$_.Set('color', $1)", color)
	p.attrs["color"] = color
	return p
}
...
func Face(face string) *Props {
	macro.Rewrite("$1().Set('face', $2)", js.Runtime("Map", "Set", "JSON"), face)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Face(face)
}
...
func (p *Props) Face(face string) *Props {
	macro.Rewrite("$_.Set('face', $1)", face)
	p.attrs["face"] = face
	return p
}
