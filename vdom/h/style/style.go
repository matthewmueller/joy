...
// New fn
func New(props *Props, children ...vdom.Child) *Style {
	js.Rewrite("$1('style', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Style) String() string {
	...
	if len(props) > 0 {
		return "<style " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</style>"
	}

	return "<style>" + strings.Join(children, "") + "</style>"
}
...
// Scoped fn
func Scoped(scoped string) *Props {
	js.Rewrite("$1().Set('scoped', $2)", js.Runtime("Map", "Set", "JSON"), scoped)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Scoped(scoped)
}

// Scoped fn
func (p *Props) Scoped(scoped string) *Props {
	js.Rewrite("$_.Set('scoped', $1)", scoped)
	p.attrs["scoped"] = scoped
	return p
}
