...
// New fn
func New(props *Props, children ...vdom.Child) *Output {
	js.Rewrite("$1('output', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Output) Render() vdom.Node {
	return s
}
...
func (s *Output) String() string {
	...
	if len(props) > 0 {
		return "<output " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</output>"
	}

	return "<output>" + strings.Join(children, "") + "</output>"
}
...
// For fn
func For(fors string) *Props {
	js.Rewrite("$1().Set('fors', $2)", js.Runtime("Map", "Set", "JSON"), fors)
	p := &Props{attrs: map[string]interface{}{}}
	return p.For(fors)
}

// For fn
func (p *Props) For(fors string) *Props {
	js.Rewrite("$_.Set('fors', $1)", fors)
	p.attrs["for"] = fors
	return p
}
