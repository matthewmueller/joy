...
// New fn
func New(props *Props, children ...vdom.Child) *Head {
	js.Rewrite("$1('head', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Head) String() string {
	...
	if len(props) > 0 {
		return "<head " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</head>"
	}

	return "<head>" + strings.Join(children, "") + "</head>"
}
...
// Profile fn
func Profile(profile string) *Props {
	js.Rewrite("$1().Set('profile', $2)", js.Runtime("Map", "Set", "JSON"), profile)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Profile(profile)
}

// Profile fn
func (p *Props) Profile(profile string) *Props {
	js.Rewrite("$_.Set('profile', $1)", profile)
	p.attrs["profile"] = profile
	return p
}
