...
// New fn
func New(props *Props, children ...vdom.Child) *Textarea {
	js.Rewrite("$1('textarea', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Textarea) Render() vdom.Node {
	return s
}

func (s *Textarea) String() string {
	// props
	var props []string
	...
	if len(props) > 0 {
		return "<textarea " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</textarea>"
	}

	return "<textarea>" + strings.Join(children, "") + "</textarea>"
}
...
// Cols fn
func Cols(cols string) *Props {
	js.Rewrite("$1().Set('cols', $2)", js.Runtime("Map", "Set", "JSON"), cols)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Cols(cols)
}

// Cols fn
func (p *Props) Cols(cols string) *Props {
	js.Rewrite("$_.Set('cols', $1)", cols)
	p.attrs["cols"] = cols
	return p
}
...
// Rows fn
func Rows(rows string) *Props {
	js.Rewrite("$1().Set('rows', $2)", js.Runtime("Map", "Set", "JSON"), rows)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Rows(rows)
}

// Rows fn
func (p *Props) Rows(rows string) *Props {
	js.Rewrite("$_.Set('rows', $1)", rows)
	p.attrs["rows"] = rows
	return p
}

// Wrap fn
func Wrap(wrap string) *Props {
	js.Rewrite("$1().Set('wrap', $2)", js.Runtime("Map", "Set", "JSON"), wrap)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Wrap(wrap)
}

// Wrap fn
func (p *Props) Wrap(wrap string) *Props {
	js.Rewrite("$_.Set('wrap', $1)", wrap)
	p.attrs["wrap"] = wrap
	return p
}
