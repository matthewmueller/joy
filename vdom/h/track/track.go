...
// New fn
func New(props *Props, children ...vdom.Child) *Track {
	js.Rewrite("$1('track', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Track) String() string {
	...
	if len(props) > 0 {
		return "<track " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</track>"
	}

	return "<track>" + strings.Join(children, "") + "</track>"
}
...
// Kind fn
func (p *Props) Kind(kind string) *Props {
...
// Srclang fn
func Srclang(srclang string) *Props {
	js.Rewrite("$1().Set('srclang', $2)", js.Runtime("Map", "Set", "JSON"), srclang)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Srclang(srclang)
}

// Srclang fn
func (p *Props) Srclang(srclang string) *Props {
	js.Rewrite("$_.Set('srclang', $1)", srclang)
	p.attrs["srclang"] = srclang
	return p
}
