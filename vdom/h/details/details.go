...
// New fn
func New(props *Props, children ...vdom.Child) *Details {
	js.Rewrite("$1('details', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Details) Render() vdom.Node {
	return s
}
...
func (s *Details) String() string {
	...
	if len(props) > 0 {
		return "<details " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</details>"
	}

	return "<details>" + strings.Join(children, "") + "</details>"
}
...
// Open fn
func Open(open string) *Props {
	js.Rewrite("$1().Set('open', $2)", js.Runtime("Map", "Set", "JSON"), open)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Open(open)
}

// Open fn
func (p *Props) Open(open string) *Props {
	js.Rewrite("$_.Set('open', $1)", open)
	p.attrs["open"] = open
	return p
}
