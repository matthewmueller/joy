...
// New fn
func New(props *Props, children ...vdom.Child) *Link {
	js.Rewrite("$1('link', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Link) String() string {
	...
	if len(props) > 0 {
		return "<link " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</link>"
	}

	return "<link>" + strings.Join(children, "") + "</link>"
}
...
// Media fn
func Media(media string) *Props {
	js.Rewrite("$1().Set('media', $2)", js.Runtime("Map", "Set", "JSON"), media)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Media(media)
}

// Media fn
func (p *Props) Media(media string) *Props {
	js.Rewrite("$_.Set('media', $1)", media)
	p.attrs["media"] = media
	return p
}
