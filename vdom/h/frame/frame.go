...
// New fn
func New(props *Props, children ...vdom.Child) *Frame {
	js.Rewrite("$1('frame', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Frame) String() string {
	...
	if len(props) > 0 {
		return "<frame " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</frame>"
	}

	return "<frame>" + strings.Join(children, "") + "</frame>"
}
...
// Noresize fn
func Noresize(noresize string) *Props {
	js.Rewrite("$1().Set('noresize', $2)", js.Runtime("Map", "Set", "JSON"), noresize)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Noresize(noresize)
}

// Noresize fn
func (p *Props) Noresize(noresize string) *Props {
	js.Rewrite("$_.Set('noresize', $1)", noresize)
	p.attrs["noresize"] = noresize
	return p
}
