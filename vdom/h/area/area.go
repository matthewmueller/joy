...
// New fn
func New(props *Props, children ...vdom.Child) *Area {
	js.Rewrite("$1('area', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Area) String() string {
	...
	if len(props) > 0 {
		return "<area " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</area>"
	}

	return "<area>" + strings.Join(children, "") + "</area>"
}
...
// Nohref fn
func Nohref(nohref string) *Props {
	js.Rewrite("$1().Set('nohref', $2)", js.Runtime("Map", "Set", "JSON"), nohref)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Nohref(nohref)
}

// Nohref fn
func (p *Props) Nohref(nohref string) *Props {
	js.Rewrite("$_.Set('nohref', $1)", nohref)
	p.attrs["nohref"] = nohref
	return p
}
