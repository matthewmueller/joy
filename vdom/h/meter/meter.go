...
// New fn
func New(props *Props, children ...vdom.Child) *Meter {
	js.Rewrite("$1('meter', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Meter) String() string {
	...
	if len(props) > 0 {
		return "<meter " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</meter>"
	}

	return "<meter>" + strings.Join(children, "") + "</meter>"
}
...
// High fn
func High(high string) *Props {
	js.Rewrite("$1().Set('high', $2)", js.Runtime("Map", "Set", "JSON"), high)
	p := &Props{attrs: map[string]interface{}{}}
	return p.High(high)
}

// High fn
func (p *Props) High(high string) *Props {
	js.Rewrite("$_.Set('high', $1)", high)
	p.attrs["high"] = high
	return p
}

// Low fn
func Low(low string) *Props {
	js.Rewrite("$1().Set('low', $2)", js.Runtime("Map", "Set", "JSON"), low)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Low(low)
}

// Low fn
func (p *Props) Low(low string) *Props {
	js.Rewrite("$_.Set('low', $1)", low)
	p.attrs["low"] = low
	return p
}
...
// Optimum fn
func Optimum(optimum string) *Props {
	js.Rewrite("$1().Set('optimum', $2)", js.Runtime("Map", "Set", "JSON"), optimum)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Optimum(optimum)
}

// Optimum fn
func (p *Props) Optimum(optimum string) *Props {
	js.Rewrite("$_.Set('optimum', $1)", optimum)
	p.attrs["optimum"] = optimum
	return p
}
