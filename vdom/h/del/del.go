...
// New fn
func New(props *Props, children ...vdom.Child) *Del {
	js.Rewrite("$1('del', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Del) Render() vdom.Node {
	return s
}

func (s *Del) String() string {
	// props
	var props []string
	...
	if len(props) > 0 {
		return "<del " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</del>"
	}

	return "<del>" + strings.Join(children, "") + "</del>"
}
...
// Cite fn
func Cite(cite string) *Props {
	js.Rewrite("$1().Set('cite', $2)", js.Runtime("Map", "Set", "JSON"), cite)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Cite(cite)
}

// Cite fn
func (p *Props) Cite(cite string) *Props {
	js.Rewrite("$_.Set('cite', $1)", cite)
	p.attrs["cite"] = cite
	return p
}

// Datetime fn
func Datetime(datetime string) *Props {
	js.Rewrite("$1().Set('datetime', $2)", js.Runtime("Map", "Set", "JSON"), datetime)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Datetime(datetime)
}

// Datetime fn
func (p *Props) Datetime(datetime string) *Props {
	js.Rewrite("$_.Set('datetime', $1)", datetime)
	p.attrs["datetime"] = datetime
	return p
}
