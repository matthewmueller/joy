...
// New fn
func New(props *Props, children ...vdom.Child) *Menuitem {
	js.Rewrite("$1('menuitem', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Menuitem) Render() vdom.Node {
	return s
}

func (s *Menuitem) String() string {
	// props
	var props []string
	...
	if len(props) > 0 {
		return "<menuitem " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</menuitem>"
	}

	return "<menuitem>" + strings.Join(children, "") + "</menuitem>"
}
...
// Default fn
func Default(def string) *Props {
	js.Rewrite("$1().Set('def', $2)", js.Runtime("Map", "Set", "JSON"), def)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Default(def)
}

// Default fn
func (p *Props) Default(def string) *Props {
	js.Rewrite("$_.Set('def', $1)", def)
	p.attrs["default"] = def
	return p
}
...
// Icon fn
func Icon(icon string) *Props {
	js.Rewrite("$1().Set('icon', $2)", js.Runtime("Map", "Set", "JSON"), icon)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Icon(icon)
}

// Icon fn
func (p *Props) Icon(icon string) *Props {
	js.Rewrite("$_.Set('icon', $1)", icon)
	p.attrs["icon"] = icon
	return p
}
...
func Label(label string) *Props {
	js.Rewrite("$1().Set('label', $2)", js.Runtime("Map", "Set", "JSON"), label)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Label(label string) *Props {
	js.Rewrite("$_.Set('label', $1)", label)
	p.attrs["label"] = label
	return p
}

// Radiogroup fn
func Radiogroup(radiogroup string) *Props {
	js.Rewrite("$1().Set('radiogroup', $2)", js.Runtime("Map", "Set", "JSON"), radiogroup)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Radiogroup(radiogroup)
}

// Radiogroup fn
func (p *Props) Radiogroup(radiogroup string) *Props {
	js.Rewrite("$_.Set('radiogroup', $1)", radiogroup)
	p.attrs["radiogroup"] = radiogroup
	return p
}
