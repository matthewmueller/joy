...
func New(props *Props, children ...vdom.Child) *Menuitem {
	macro.Rewrite("$1('menuitem', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Default(def string) *Props {
	macro.Rewrite("$1().Set('def', $2)", js.Runtime("Map", "Set", "JSON"), def)
	p := &Props{attrs: map[string]interface{}{}}
...
func Icon(icon string) *Props {
	macro.Rewrite("$1().Set('icon', $2)", js.Runtime("Map", "Set", "JSON"), icon)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Icon(icon)
}
...
func (p *Props) Icon(icon string) *Props {
	macro.Rewrite("$_.Set('icon', $1)", icon)
	p.attrs["icon"] = icon
	return p
}
...
func Radiogroup(radiogroup string) *Props {
	macro.Rewrite("$1().Set('radiogroup', $2)", js.Runtime("Map", "Set", "JSON"), radiogroup)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Radiogroup(radiogroup string) *Props {
	macro.Rewrite("$_.Set('radiogroup', $1)", radiogroup)
