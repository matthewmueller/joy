...
// New fn
func New(props *Props, children ...vdom.Child) *Colgroup {
	js.Rewrite("$1('colgroup', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Colgroup) Render() vdom.Node {
	return s
}

func (s *Colgroup) String() string {
	// props
	var props []string
	...
	if len(props) > 0 {
		return "<colgroup " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</colgroup>"
	}

	return "<colgroup>" + strings.Join(children, "") + "</colgroup>"
}
...
// Span fn
func Span(span string) *Props {
	js.Rewrite("$1().Set('span', $2)", js.Runtime("Map", "Set", "JSON"), span)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Span(span)
}

// Span fn
func (p *Props) Span(span string) *Props {
	js.Rewrite("$_.Set('span', $1)", span)
	p.attrs["span"] = span
	return p
}
