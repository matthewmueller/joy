...
// New fn
func New(props *Props, children ...vdom.Child) *Script {
	js.Rewrite("$1('script', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Script) Render() vdom.Node {
	return s
}
...
func (s *Script) String() string {
	...
	if len(props) > 0 {
		return "<script " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</script>"
	}

	return "<script>" + strings.Join(children, "") + "</script>"
}
...
// Async fn
func Async(async string) *Props {
	js.Rewrite("$1().Set('async', $2)", js.Runtime("Map", "Set", "JSON"), async)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Async(async)
}

// Async fn
func (p *Props) Async(async string) *Props {
	js.Rewrite("$_.Set('async', $1)", async)
	p.attrs["async"] = async
	return p
}
...
// Defer fn
func Defer(dfr string) *Props {
	js.Rewrite("$1().Set('dfr', $2)", js.Runtime("Map", "Set", "JSON"), dfr)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Defer(dfr)
}

// Defer fn
func (p *Props) Defer(dfr string) *Props {
	js.Rewrite("$_.Set('dfr', $1)", dfr)
	p.attrs["defer"] = dfr
	return p
}

// Language fn
func Language(language string) *Props {
	js.Rewrite("$1().Set('language', $2)", js.Runtime("Map", "Set", "JSON"), language)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Language(language)
}

// Language fn
func (p *Props) Language(language string) *Props {
	js.Rewrite("$_.Set('language', $1)", language)
	p.attrs["language"] = language
	return p
}

// Nonce fn
func Nonce(nonce string) *Props {
	js.Rewrite("$1().Set('nonce', $2)", js.Runtime("Map", "Set", "JSON"), nonce)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Nonce(nonce)
}

// Nonce fn
func (p *Props) Nonce(nonce string) *Props {
	js.Rewrite("$_.Set('nonce', $1)", nonce)
	p.attrs["nonce"] = nonce
	return p
}
