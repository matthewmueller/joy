...
// New fn
func New(props *Props, children ...vdom.Child) *Button {
	js.Rewrite("$1('button', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Button) Render() vdom.Node {
	return s
}
...
func (s *Button) String() string {
	...
	if len(props) > 0 {
		return "<button " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</button>"
	}

	return "<button>" + strings.Join(children, "") + "</button>"
}
...
// Menu fn
func Menu(menu string) *Props {
	js.Rewrite("$1().Set('menu', $2)", js.Runtime("Map", "Set", "JSON"), menu)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Menu(menu)
}

// Menu fn
func (p *Props) Menu(menu string) *Props {
	js.Rewrite("$_.Set('menu', $1)", menu)
	p.attrs["menu"] = menu
	return p
}
