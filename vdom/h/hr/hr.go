...
func New(props *Props, children ...vdom.Child) *Hr {
	js.Rewrite("$1('hr', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Noshade fn
func Noshade(noshade string) *Props {
	js.Rewrite("$1().Set('noshade', $2)", js.Runtime("Map", "Set", "JSON"), noshade)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Noshade(noshade)
}

// Noshade fn
func (p *Props) Noshade(noshade string) *Props {
	js.Rewrite("$_.Set('noshade', $1)", noshade)
	p.attrs["noshade"] = noshade
	return p
}
