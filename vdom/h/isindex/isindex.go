...
// New fn
func New(props *Props, children ...vdom.Child) *Isindex {
	js.Rewrite("$1('isindex', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
// Render fn
func (s *Isindex) Render() vdom.Node {
	return s
}

func (s *Isindex) String() string {
	// props
	var props []string
	...
	if len(props) > 0 {
		return "<isindex " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</isindex>"
	}

	return "<isindex>" + strings.Join(children, "") + "</isindex>"
}
...
// Prompt fn
func Prompt(prompt string) *Props {
	js.Rewrite("$1().Set('prompt', $2)", js.Runtime("Map", "Set", "JSON"), prompt)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Prompt(prompt)
}

// Prompt fn
func (p *Props) Prompt(prompt string) *Props {
	js.Rewrite("$_.Set('prompt', $1)", prompt)
	p.attrs["prompt"] = prompt
	return p
}
