...
// New fn
func New(props *Props, children ...vdom.Child) *Meta {
	js.Rewrite("$1('meta', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func (s *Meta) String() string {
	...
	if len(props) > 0 {
		return "<meta " + strings.Join(props, " ") + ">" + strings.Join(children, "") + "</meta>"
	}

	return "<meta>" + strings.Join(children, "") + "</meta>"
}
...
// Content fn
func Content(content string) *Props {
	js.Rewrite("$1().Set('content', $2)", js.Runtime("Map", "Set", "JSON"), content)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Content(content)
}

// Content fn
func (p *Props) Content(content string) *Props {
	js.Rewrite("$_.Set('content', $1)", content)
	p.attrs["content"] = content
	return p
}

// HTTPEquiv fn
func HTTPEquiv(httpequiv string) *Props {
	js.Rewrite("$1().Set('httpequiv', $2)", js.Runtime("Map", "Set", "JSON"), httpequiv)
	p := &Props{attrs: map[string]interface{}{}}
	return p.HTTPEquiv(httpequiv)
}

// HTTPEquiv fn
func (p *Props) HTTPEquiv(httpequiv string) *Props {
	js.Rewrite("$_.Set('httpequiv', $1)", httpequiv)
	p.attrs["http-equiv"] = httpequiv
	return p
}
...
// Scheme fn
func Scheme(scheme string) *Props {
	js.Rewrite("$1().Set('scheme', $2)", js.Runtime("Map", "Set", "JSON"), scheme)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Scheme(scheme)
}

// Scheme fn
func (p *Props) Scheme(scheme string) *Props {
	js.Rewrite("$_.Set('scheme', $1)", scheme)
	p.attrs["scheme"] = scheme
	return p
}
