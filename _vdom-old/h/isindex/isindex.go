...
func New(props *Props, children ...vdom.Child) *Isindex {
	macro.Rewrite("$1('isindex', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Prompt(prompt string) *Props {
	macro.Rewrite("$1().Set('prompt', $2)", js.Runtime("Map", "Set", "JSON"), prompt)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Prompt(prompt string) *Props {
	macro.Rewrite("$_.Set('prompt', $1)", prompt)
