...
func New(props *Props, children ...vdom.Child) *Meta {
	macro.Rewrite("$1('meta', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Content(content string) *Props {
	macro.Rewrite("$1().Set('content', $2)", js.Runtime("Map", "Set", "JSON"), content)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Content(content string) *Props {
	macro.Rewrite("$_.Set('content', $1)", content)
...
func HTTPEquiv(httpequiv string) *Props {
	macro.Rewrite("$1().Set('httpequiv', $2)", js.Runtime("Map", "Set", "JSON"), httpequiv)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) HTTPEquiv(httpequiv string) *Props {
	macro.Rewrite("$_.Set('httpequiv', $1)", httpequiv)
...
func Scheme(scheme string) *Props {
	macro.Rewrite("$1().Set('scheme', $2)", js.Runtime("Map", "Set", "JSON"), scheme)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Scheme(scheme string) *Props {
	macro.Rewrite("$_.Set('scheme', $1)", scheme)
