...
func New(props *Props, children ...vdom.Child) *Textarea {
	macro.Rewrite("$1('textarea', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Cols(cols string) *Props {
	macro.Rewrite("$1().Set('cols', $2)", js.Runtime("Map", "Set", "JSON"), cols)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Cols(cols)
}
...
func (p *Props) Cols(cols string) *Props {
	macro.Rewrite("$_.Set('cols', $1)", cols)
	p.attrs["cols"] = cols
	return p
}
...
func Rows(rows string) *Props {
	macro.Rewrite("$1().Set('rows', $2)", js.Runtime("Map", "Set", "JSON"), rows)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Rows(rows)
}
...
func (p *Props) Rows(rows string) *Props {
	macro.Rewrite("$_.Set('rows', $1)", rows)
	p.attrs["rows"] = rows
	return p
}
...
func Wrap(wrap string) *Props {
	macro.Rewrite("$1().Set('wrap', $2)", js.Runtime("Map", "Set", "JSON"), wrap)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Wrap(wrap)
}
...
func (p *Props) Wrap(wrap string) *Props {
	macro.Rewrite("$_.Set('wrap', $1)", wrap)
	p.attrs["wrap"] = wrap
	return p
}
