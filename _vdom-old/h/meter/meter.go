...
func New(props *Props, children ...vdom.Child) *Meter {
	macro.Rewrite("$1('meter', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func High(high string) *Props {
	macro.Rewrite("$1().Set('high', $2)", js.Runtime("Map", "Set", "JSON"), high)
	p := &Props{attrs: map[string]interface{}{}}
	return p.High(high)
}
...
func (p *Props) High(high string) *Props {
	macro.Rewrite("$_.Set('high', $1)", high)
	p.attrs["high"] = high
	return p
}
...
func Low(low string) *Props {
	macro.Rewrite("$1().Set('low', $2)", js.Runtime("Map", "Set", "JSON"), low)
	p := &Props{attrs: map[string]interface{}{}}
	return p.Low(low)
}
...
func Optimum(optimum string) *Props {
	macro.Rewrite("$1().Set('optimum', $2)", js.Runtime("Map", "Set", "JSON"), optimum)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Optimum(optimum string) *Props {
	macro.Rewrite("$_.Set('optimum', $1)", optimum)
