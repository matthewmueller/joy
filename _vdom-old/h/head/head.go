...
func New(props *Props, children ...vdom.Child) *Head {
	macro.Rewrite("$1('head', $2 ? $2.JSON() : {}, $3)", vdom.Pragma(), props, children)
	if props == nil {
...
func Profile(profile string) *Props {
	macro.Rewrite("$1().Set('profile', $2)", js.Runtime("Map", "Set", "JSON"), profile)
	p := &Props{attrs: map[string]interface{}{}}
...
func (p *Props) Profile(profile string) *Props {
	macro.Rewrite("$_.Set('profile', $1)", profile)
